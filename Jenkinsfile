        ...
        stage('Commands') {
            steps {
                script {
                    ...
                    def mvCommand = 'pwd && ls -al && cp -R /var/lib/jenkins/workspace/go-pipeline/* /var/www/html/AutomationService && cd /var/www/html/AutomationService && go build && sudo su && systemctl restart nginx.service'
                    sh mvCommand
                }
            }
        }
