...
func TrendFollowingRsi(data *DataWithIndicators, token, symbol, username string, client *smartapigo.Client) ORDER {
	...
	if IsOBVIncreasing(obv) && data.Data[idx].Close > high && data.Data[idx-1].Low > ema21 && data.Data[idx].Close > GetVwap(data.Data, 14) && adx14.PlusDi[idx] > adx14.MinusDi[idx] && ma5 > ma8 && ma8 > ma13 && ma21 < ma13 && rsiAvg3 > rsiavg8 && rsi[idx] < 50 {
		order = ORDER{
	...
	} else if IsOBVDecreasing(obv) && data.Data[idx-1].High < ema21 && data.Data[idx].Close <= low && data.Data[idx].Close < GetVwap(data.Data, 14) && adx14.PlusDi[idx] < adx14.MinusDi[idx] && ma5 < ma8 && ma8 < ma13 && ma21 > ma13 && rsiAvg3 < rsiavg8 && rsi[idx] < 50 {
		order = ORDER{
